...
void detect_collision(GameObjects *go, GameControls *gc) {
    ...
    for (int i = 0; i < sizeof(go->player_lasers) / sizeof(go->player_lasers[0]); i++) {
        for (int j = 0; j < sizeof(go->enemies) / sizeof(go->enemies[0]); j++) {
            if (go->player_lasers[i].is_visible && go->enemies[j].is_visible
                && go->player_lasers[i].rect.x > go->enemies[j].rect.x - go->player_lasers[i].rect.w
                && go->player_lasers[i].rect.x < go->enemies[j].rect.x + go->enemies[j].rect.w
                && go->player_lasers[i].rect.y < go->enemies[j].rect.y + go->enemies[j].rect.h) {
                // Set explosion
    ...
    for (int i = 0; i < sizeof(go->enemy_lasers) / sizeof(go->enemy_lasers[0]); i++) {
        if (go->enemy_lasers[i].is_visible
            && go->enemy_lasers[i].rect.x > go->player.rect.x - go->enemy_lasers[i].rect.w
    ...
    for (int i = 0; i < sizeof(go->player_lasers) / sizeof(go->player_lasers[0]); i++) {
        for (int j = 0; j < sizeof(go->asteroids) / sizeof(go->asteroids[0]); j++) {
            if (go->player_lasers[i].is_visible && go->asteroids[j].is_visible
                && go->player_lasers[i].rect.x > go->asteroids[j].rect.x - go->player_lasers[i].rect.w
                && go->player_lasers[i].rect.x < go->asteroids[j].rect.x + go->asteroids[j].rect.w
                && go->player_lasers[i].rect.y < go->asteroids[j].rect.y + go->asteroids[j].rect.h) {
                // Set explosion
    ...
    for (int i = 0; i < sizeof(go->enemies) / sizeof(go->enemies[0]); i++) {
        if (go->enemies[i].is_visible
            && go->player.rect.x > go->enemies[i].rect.x - go->player.rect.w
            && go->player.rect.x < go->enemies[i].rect.x + go->enemies[i].rect.w
            && go->player.rect.y < go->enemies[i].rect.y + go->enemies[i].rect.h
            && go->player.rect.y > go->enemies[i].rect.y - go->player.rect.h) {
            go->enemies[i].y = -150;
        }
    }
    ...
    for (int i = 0; i < sizeof(go->asteroids) / sizeof(go->asteroids[0]); i++) {
        if (go->asteroids[i].is_visible
            && go->player.rect.x > go->asteroids[i].rect.x - go->player.rect.w
            && go->player.rect.x < go->asteroids[i].rect.x + go->asteroids[i].rect.w
            && go->player.rect.y < go->asteroids[i].rect.y + go->asteroids[i].rect.h
            && go->player.rect.y > go->asteroids[i].rect.y - go->player.rect.h) {
